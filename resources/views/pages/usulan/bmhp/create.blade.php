...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-12">
                <h4 class="m-0">Usulan BMHP & Alat Kesehatan</h4>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item active"><a href="{{ route('dashboard') }}"> Dashboard</a></li>
                    <li class="breadcrumb-item active"><a href="{{ route('usulan','bmhp') }}"> Daftar</a></li>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-1 form-group">
                        <a type="button" class="btn btn-app btn-default btn-block border-dark" data-toggle="modal" data-target="#basket">
                            <span class="badge bg-danger mr-3" id="cart-count">{{ Auth::user()->keranjangBmhp->count() }}</span>
                    ...
                    <div class="form-group col-md-2 col-6" data-kategori="{{ $row->kategori_id }}">
                        <div class="card card-psdia">
                            <div class="card-header">
                                @if ($row->foto_barang)
                                <img src="{{ asset('dist/img/foto_bmhp/'. $row->foto_barang) }}" class="img-fluid" alt="">
                                @else
                            ...
                            <div class="card-footer">
                                ...
                                <center class="text-xs">
                                    <label class="text-secondary" id="jumlahPcs-{{ $row->id_bmhp }}">Total: 0 <small>pcs</small></label>
                                </center>
                                @endif
                                <form id="form-{{ $row->id_bmhp }}" action="{{ route('bmhp-bucket.create') }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="bmhp_id" value="{{ $row->id_bmhp }}">
                                    <input type="hidden" name="proses" value="keranjang">
                                    <div class="input-group">
                                        <a type="button" class="min-button" data-id="data-{{ $row->id_bmhp }}">
                                        ...
                                        <input type="text" class="form-control form-control-sm text-center bg-white number qty-input"
                                            ...
                                            oninput="updateTotal('{{ $row->id_bmhp }}')"
                                            @if(Auth::user()->role_id == 4) max="{{ $row->jumlah_maks }}" @endif>

                                        <a type="button" class="add-button" data-id="data-{{ $row->id_bmhp }}">
                                            <div class="input-group-append">
                                    ...
                                    <button class="btn btn-outline-danger btn-block btn-sm add-to-cart-button mt-2 {{ $row->status != 'true' ? 'disabled' : '' }}" data-id="{{ $row->id_bmhp }}">
...
<!-- Modal -->
<form id="form" action="{{ route('usulan.store', 'bmhp') }}" method="POST">
    @csrf
        ...
        <div class="modal-dialog modal-dialog-scrollable modal-xl" role="document">
            <div class="modal-content">
                ...
                <div class="modal-header border-dark">
                    <h5 class="modal-title text-md">KERANJANG SAYA (<span id="cart-count-in">{{ Auth::user()->keranjangBmhp->count() }}</span> item)</h5>
                ...
                <div class="modal-body table-responsive">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 text-sm mt-2">
                                ...
                                <div class="d-flex">
                                    ...
                                    <span class="w-75">: Permintaan BMHP & Alkes</span>
                                </div>
                                <div class="d-flex">
                    ...
                    <div id="cart-items">
                        @foreach (Auth::user()->keranjangBmhp as $row)
                        <input type="hidden" name="id_keranjang[]" value="{{ $row->id_keranjang }}">
                        <input type="hidden" name="id_bmhp[]" value="{{ $row->bmhp_id }}">
                        <table class="table table-bordered text-center text-sm m-1">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td class="align-middle">
                                        @if ($row->bmhp->foto_barang)
                                        <img src="{{ asset('dist/img/foto_bmhp/'. $row->bmhp->foto_barang) }}" class="img-fluid" alt="">
                                        @else
                                        <img src="https://cdn-icons-png.flaticon.com/512/679/679821.png" class="img-fluid" alt="">
                                        @endif
                                    </td>
                                    <td class="align-middle text-left">
                                        {{ $row->bmhp->kategori->nama_kategori }}
                                        ...
                                        <div class="row">
                                            <div class="col-md-12 col-12">
                                                <div class="input-group-text rounded-left" style="height: 31px;">
                                                    <a type="button" class="qty-button-modal col-12" data-id="{{ $row->id_keranjang }}" href="{{ route('bmhp-bucket.update', ['aksi' => 'plus', 'id' => $row->id_keranjang]) }}">
                                            ...
                                            <div class="col-md-12 col-12">
                                                <div class="input-group-text rounded-left" style="height: 31px;">
                                                    <a type="button" class="qty-button-modal col-12" data-id="{{ $row->id_keranjang }}" href="{{ route('bmhp-bucket.update', ['aksi' => 'min', 'id' => $row->id_keranjang]) }}">
                                    ...
                                    <td><label>{{ $row->bmhp->satuan->nama_satuan }}</label></td>
                                    <td class="text-left">
                        ...
                        @endforeach
                        @if(Auth::user()->keranjangBmhp->count() == 0)
                        <table class="table table-bordered text-center text-sm m-1">
                            <tr>
...
<script>
    ...
    function updateCartItems(cartBasket) {
        ...
        $.each(cartBasket, function(index, item) {
            const updateMin = '{{ route("bmhp-bucket.update", ["aksi" => "min", "id" => "__ID__"]) }}'.replace('__ID__', item.id_keranjang)
            const updateAdd = '{{ route("bmhp-bucket.update", ["aksi" => "add", "id" => "__ID__"]) }}'.replace('__ID__', item.id_keranjang)
            const removeItem = '{{ route("bmhp-bucket.remove", ["id" => "__ID__"]) }}'.replace('__ID__', item.id_keranjang)
            const satuan = ''
            ...
            const cartItem = `
                <table class="table table-bordered text-center text-sm m-1">
                    ...
                    <input type="hidden" name="id_bmhp[]" value="${item.bmhp_id}">
                    <thead>
                        <tr>
                            <th style="width: 5%;">No</th>
                            <th style="width: 5%;" class="p-2">Foto</th>
    ...
    function confirmRemove(itemId) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    type: 'GET',
                    url: '{{ route("bmhp-bucket.remove", ["id" => "__ID__"]) }}'.replace('__ID__', itemId),
                    success: function(response) {
    ...
    function confirmSubmit(event) {
        ...
        const basket = '{{ Auth::user()->keranjangBmhp->count() }}'
...
<script>
    function updateTotal(bmhpId) {
        let qtyInput = document.getElementById("data-" + bmhpId);
        ...
        document.getElementById("jumlahPcs-" + bmhpId).textContent = "Total: " + totalPcs + " pcs";
    }
