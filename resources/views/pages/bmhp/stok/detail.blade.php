        ...
        <div class="card border border-dark">
            <div class="card-header">
                ...
                <div class="card-tools mt-2">
                    <a href="{{ route('bmhp-stok.edit', $data->id_stok) }}" class="btn btn-warning border-dark btn-xs mt-0 p-1">
                        <i class="fas fa-edit"></i> Edit
                    </a>

                    <a href="#" class="btn btn-danger border-dark btn-xs mt-0 p-1" onclick="confirmLink(event, `{{ route('bmhp-stok.delete', $data->id_stok) }}`)">
                    ...
                    <table id="table" class="table table-bordered border border-dark">
                        ...
                        <tbody class="text-xs">
                            ...
                            <tr class="bg-white">
                                ...
                                <td>{{ $row->bmhp->kategori->nama_kategori }} {{ $row->bmhp->nama_barang }}</td>
                                <td>{{ $row->bmhp->deskripsi }}</td>
                                <td class="text-center">{{ number_format($row->jumlah, 0, '.') }}</td>
                                <td class="text-center">{{ $row->bmhp->satuan->nama_satuan }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
